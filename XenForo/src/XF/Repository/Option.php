...
class Option extends Repository
{
	...
	public function refreshEmailAccessTokenIfNeeded($optionId, &$optionUpdated = false)
	{
		...
		$option = $this->em->find('XF:Option', $optionId);
		if (!$option)
		{
		...
		{
			throw new \LogicException("Option {$option->option_id} does not store the access token in a known place");
		}
		...
		$this->options()->{$option->option_id} = $option->option_value;
